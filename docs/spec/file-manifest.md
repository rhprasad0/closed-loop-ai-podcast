...
| `lambdas/shared/python/shared/bedrock.py` | Bedrock client wrapper. Functions: `invoke_model(user_message, system_prompt, model_id, max_tokens, effort)`, `invoke_with_tools(user_message, system_prompt, tools, tool_executor, model_id, max_tokens, max_turns, effort)`. Default model: Claude on Bedrock. Handles retries for throttling. |
...
| `lambdas/shared/python/shared/s3.py` | S3 helper functions: `upload_bytes(bucket, key, data, content_type)`, `upload_file(bucket, key, filepath, content_type)`, `download_file(bucket, key, local_path)`, `generate_presigned_url(bucket, key, expiry)`. Callers pass `bucket` explicitly (read from `S3_BUCKET` env var at the call site). |
