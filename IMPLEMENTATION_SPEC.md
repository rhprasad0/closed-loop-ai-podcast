...
The state object grows as it passes through the pipeline:

```jsonc
{
  // Written by Discovery Lambda
  "discovery": { ... },

  // Written by Research Lambda
  "research": { ... },

  // Written by Script Lambda
  "script": { ... },

  // Written by Producer Lambda
  "producer": { ... },

  // Written by Cover Art Lambda
  "cover_art": { ... },

  // Written by TTS Lambda
  "tts": { ... },

  // Written by Post-Production Lambda
  "post_production": { ... },

  // Pipeline metadata (managed by Step Functions)
  "metadata": {
    "execution_id": "string",
    "script_attempt": 1
  }
...
**Input:** Initial state (empty or with `metadata` only).
...
**Input:** State with `discovery` populated.
...
**Input:** State with `discovery` and `research` populated. If this is a retry, `producer.feedback` will also be present and `metadata.script_attempt` will be > 1.
...
{
  "script": {
    "text": "The full script text with speaker labels in **Speaker:** format",
...
**Input:** Full state with `discovery`, `research`, and `script` populated.
...
**Input:** Full state through `producer` (PASS).
...
{
  "cover_art": {
    "s3_key": "episodes/ep-002/cover.png",
...
**Input:** Full state through `cover_art`.
...
{
  "tts": {
    "s3_key": "episodes/ep-002/episode.mp3",
...
{
  "post_production": {
    "s3_mp4_key": "episodes/ep-002/episode.mp4",
    "episode_id": 2,
    "air_date": "2025-07-13"
  }
}
